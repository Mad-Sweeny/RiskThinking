...
df_market_data = spark.read.parquet(r'/home/ebony_maw_0/data/processed_output.parquet') 
...
df_market_data    .select('Symbol','Security_Name','Date','Open','High','Low','Close','Adj_Close','Volume','vol_moving_avg','adj_close_rolling_med')    .coalesce(1)    .write    .format('parquet')    .mode('overwrite')    .save(r'/home/ebony_maw_0/data/feature_engineering_output.parquet')


print('Parquet writing complete. Starting csv writing....')

#df_market_data.select('Date','Volume','vol_moving_avg','adj_close_rolling_med').coalesce(1).write.mode('overwrite').save(r'/home/ebony_maw_0/data/model_training_data/',format='csv')
df_market_data.select('Date','Volume','vol_moving_avg','adj_close_rolling_med').coalesce(1).write.mode('overwrite').option('header', 'true').csv('/home/ebony_maw_0/data/model_training_data/')
